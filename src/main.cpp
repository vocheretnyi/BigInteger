...
void solveGauss() {
    const string path = "C:\\Users\\Viacheslav\\BigInteger\\BigInteger\\src\\gaussInputExample3.txt";
    ifstream cin(path);
    int n = 0, m = 0;
    cin >> n >> m;
    // n - кол-во уравнени
    ...
    cout << "Number of equation: " << n << "\n";
    cout << "Number of variables: " << m << "\n";
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            cout << setw(5) << right << a[i][j].getNumerator() << " * x_" << (j+1);
            if (j + 1 < m) {
                cout << " + ";
            }
        }
    ...
    cout << "\n";
    int res = gauss(a, ans);
    cout << "Number of system solutions: ";
    if (res != INF) {
        cout << res << "\n";
    } else {
        cout << "INF\n";
    ...
    if (res != 0) {
        for (int i = 0; i < ans.size(); ++i) {
            cout << "x_" << (i + 1) << " = " << ans[i].getNumerator() << '/' << ans[i].getDenominator() << ",\n";
        }
    }
}
